...
-------
>>> import pandas as pd
>>> from gnvae.networks.module_extraction import load_adjacency, leiden_modules, compute_module_eigengenes
>>> adjacency, genes = load_adjacency("adjacency.npz")
...
from gnvae.networks.utils import transform_adjacency_for_clustering
...
def _infer_separator(path: str) -> str:
    """Infer CSV/TSV separator from file extension."""
...
def _ensure_array_and_genes(
    ...
    """Convert adjacency to array and extract gene list."""
...
def load_adjacency(path: str, genes_path: Optional[str] = None) -> Tuple[np.ndarray, List[str]]:
    ...
    genes_path:
        Optional path to gene lookup file for legacy formats.
    ...
    if ".npz" in suffixes:
        from gnvae.networks.extract_networks import load_adjacency_sparse
        return load_adjacency_sparse(path)

    # Handle Parquet edge list format
    if ".parquet" in suffixes:
        from gnvae.networks.extract_networks import load_edge_list_parquet
...
def build_graph_from_adjacency(A: np.ndarray | pd.DataFrame, genes: Optional[Sequence[str]] = None):
    ...
    try:
        ...
        raise ImportError(
            "igraph is required for graph construction. "
...
def leiden_modules(
    ...
    *,
    ...
    adjacency_mode:
        How to handle negative edge weights:
        - "wgcna-signed" (default): clip negative values to zero
        - "signed": preserve negative edges (NOT supported by Leiden)

    Notes
    -----
    Leiden does not support negative edge weights. By default, GNVAE uses a
    ...
    ------
    ImportError
        If leidenalg or igraph is not installed. Install with:
        ``pip install gnvae[clustering]``
    """
    try:
        import leidenalg
    except ImportError as exc:
        raise ImportError(
            "leidenalg is required for Leiden clustering. "
            "Install with: pip install gnvae[clustering]"
        ) from exc
...
def compute_module_eigengenes(datExpr: pd.DataFrame, modules: Mapping[str, int]) -> pd.DataFrame:
    ...
    -------
    pandas.DataFrame
        Samples x modules matrix of eigengene values.
    """
    module_series = pd.Series(modules, name="module")
...
def save_eigengenes(eigengenes: pd.DataFrame, output_path: str) -> None:
    ...
    ----------
    eigengenes:
        Samples x modules DataFrame produced by :func:`compute_module_eigengenes`.
    output_path:
        Destination CSV/TSV path.
    """
    path = Path(output_path)
